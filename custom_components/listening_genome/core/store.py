...
enrichment path (:mod:`..enrich.musicbrainz`) needs somewhere to
...
class ArtistMetaWrite(TypedDict, total=False):
    ...
    A superset of :class:`~.models.ArtistMeta` (the frozen
...
class GenomeStore:
    ...
    def db_path(self) -> str:
        ...
        return os.path.join(self.storage_path, "genome.db")
    ...
    async def snapshot_to(self, target: str) -> None:
        ...
        In the Music Assistant fork it had to be this connection: MA opened every database with
        ``PRAGMA locking_mode=exclusive`` over WAL, so no second connection could read the file
        at all, and the export spent two rounds waiting on a lock that could not be granted.
        :class:`~.database.GenomeDatabase` drops exclusive mode, so a second reader now works,
        but ``VACUUM INTO`` on the owning connection is still the right tool: it produces a
        compacted, transactionally consistent file without a second connection. It cannot run inside a transaction, so anything pending
        is committed first.
    ...
    def __init__(
        ...
        storage_path: str,
        *,
        player_name_resolver: Callable[[str], str | None] | None = None,
    ) -> None:
        """
        Initialize the store.

        :param storage_path: Directory that holds (or will hold) ``genome.db``.
        :param player_name_resolver: Maps a stored ``player_id`` to a display name, or ``None``
            when it is unknown. Used only by :meth:`player_names`; without one, every player is
            shown by its id.
        """
    ...
    async def setup(self) -> None:
        ...
        self.database = GenomeDatabase(db_path)
